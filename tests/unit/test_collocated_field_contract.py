...
**This file was named `test_field_contract.py` until 2026-08-22**, when
the Stage 2 exit audit found that reading as a claim it did not make.
Every assertion here -- `values`, `component_shape`, cell-indexed access
-- is about *collocated* storage, so a staggered placement could never
pass it, yet Stage 2 Completion Criterion 2 promised a suite "any future
implementation (e.g. a staggered placement) must pass unchanged". That
promise now belongs to `test_field_contract.py`, which asserts only what
`Field` itself declares; this file keeps the collocated half, under a
name that says so. A collocated implementation must pass both.
"""
