...
### The repository manifest, split rather than generated

Maintainer's call, and the framing was the useful part: if we cannot
generate a manifest of what is in the repository, what are we doing. The
answer turned out to be that `docs/repository-manifest.md` does two jobs
and only one of them is generatable.

*What* is in the repository is a fact, and a fact restated by hand goes
stale. It has now done so twice: v0.1 described roughly 35 handbook files
that never existed while omitting most of the repository, and even after
the 2026-08-15 rewrite the document spent five days calling `src/`
"docstring-only, no implementation" against 1,470 lines of code. That
half is now `docs/repository-inventory.md`, generated by `make inventory`
from `git ls-files` and checked in `make ci`.

*Why* each artifact exists -- what it is for, what was deliberately left
out, what was retired and on what grounds -- is judgement and
institutional memory. No generator produces "run for real 2026-08-15
(B4) -- fixed two files on first run, clean on second", and that
sentence is most of the reason the document is worth having. That half
stays hand-written.

Splitting rather than merging follows the precedent already in the
repository: `docs/index.md` is the generated navigation map and
`README.md`'s "Where to Start" is the curated path, cross-linked rather
than combined (`docs/documentation-guidelines.md`, single primary
purpose per doc). Mixing generated and hand-written content in one file
means either the generator overwrites prose or a human edits inside
generated output, and the root `CLAUDE.md` rule exists to prevent the
second of those.

`make check-manifest` is what stops the hand-written half drifting:
every tracked file must be named in the manifest -- by path or basename,
since the tables list bare filenames -- or matched by a collective rule
declared in the manifest itself, in a fenced `collective-coverage`
block. Declared there rather than in the script so the document's own
statement of its contract and the mechanism enforcing it cannot separate
(P-011). Five globs were needed; the manifest already covered 163 of 178
tracked files by name.

**The rule that was removed is the more useful record.** A fourth rule
-- "every path the manifest names exists" -- was written, run against the
real manifest, and deleted before shipping. It produced 44 findings and
essentially all were false: the manifest names `tools/planner/`,
`assets/textures/` and `docs/planning/numerical-frameworks.md` on
purpose, because recording what was retired and why is part of its job,
and it writes section-relative paths and at least one naming template
(`ADR-00N-title.md`). Separating those from a genuinely stale reference
needs a reader.

That is `tools/validators/CLAUDE.md`'s gate-versus-advisory question in a
new form. The rule was not wrong; it was not *gate-shaped*. Three
options: make the whole validator advisory and lose the coverage check's
teeth, add suppressions until the noise stopped -- which is exactly how
an exemption list stops meaning anything, the trap `check_claims.py`
already documents -- or drop the rule. Dropped, with a named test
pinning the decision so it is not re-added without someone seeing why it
went. **Prefer three rules that always mean something to four where one
needs interpreting.**

**What this does not fix, stated plainly**: the "42 tests, 87% coverage"
claim that prompted the whole thing. That number comes from running the
suite, not from listing files, so no file-inventory generator could ever
have produced it. Those claims stay prose with a date attached, and step
11 of the end-of-session review -- now updated to say which half is
mechanised and which is not -- remains the only thing covering them. A
green `make ci` does not mean the counts are current.

- *Verified by:* `make ci` clean with both new targets -- 202 tests, up
...
### Stage 2 completion criteria drafted, before TASK-014 starts

Applying `docs/practices.md`'s "a stage gets completion criteria before
its first task" rule for the first time since it was written -- Stage 1
closed without any, five of the eight retrospective criteria were found
unmet, and the rule exists precisely so that does not happen again.
Stage 2 (Representing Fields) had a one-line Goal and nothing else;
TASK-014 (Field Interface) has not started, so this was the point to
write them, not after.

Nine criteria, `docs/planning/roadmap.md`, deliberately about the
stage's goal rather than a restatement of TASK-014..017's own Acceptance
Criteria -- the specific mistake the rule was written to prevent. Two
worth calling out: a field's storage shape must be tied to its mesh by
construction, not merely agree with it during testing (the same
confident-wrong-answer failure mode `InvalidMeshEntityError` closed for
`Mesh` itself); and initialisation must be general
callable/expression-based, not a uniform constant or a closed preset
list -- already decided under TASK-015 on 2026-08-20, restated here as a
stage-level criterion because Stage 6's validation demos (Taylor-Green
vortex, Kelvin-Helmholtz, Rayleigh-Bénard) depend on it directly and it
is cheap now, expensive to retrofit once those demos exist. Explicitly
carries no physical-correctness extension (`docs/practices.md`) -- Stage
2 stores and displays data, it solves no equation yet -- the same
carve-out TASK-013 stated for itself as pure rendering.

- *Verified by:* no code changed; `make check-docs` and `make lint` run
  clean over the edited file.

### TASK-014..017 given real Acceptance Criteria, before any of their code exists

TASK-014 through TASK-017 had never had more than an `Implement:` bullet
list -- the Stage 3+ level of planning, not the level TASK-011/012/013
were each held to. Asked directly whether Stage 2's tasks were fully
specified; they weren't, and TASK-015's own text already carried a
standing IOU saying as much ("write this into this task's own acceptance
criteria when it's reached"). Written now, before TASK-014 starts,
matching TASK-011/012/013's own Purpose/Dependencies/Artifacts
Produced/Implementation/Acceptance Criteria shape.

**One real design correction came out of writing them, not before.** A
first pass put cell-count-shaped tensor allocation directly on `Field`
itself, for convenience. That would have made `Field` silently assume
collocated (cell-centred) storage -- exactly the assumption its own
existing text says the interface "deliberately does not" make, since
collocated-vs-staggered is precisely a disagreement about how values map
onto the mesh. Caught while drafting, not after: `Field` now carries only
`mesh`, `name`, and an abstract `copy()`; a new `CollocatedField`
(`src/pyflow/engine/collocated_field.py`, TASK-015) is where cell-centred
storage, the generic initialiser, and `value_at`/`set_value_at` actually
live, shared by `ScalarField` and `VectorField` alike. This also turned
out to match a detail the original TASK-014 text already had and this
session hadn't been reading closely enough: it names `CollocatedField`
as arriving with TASK-015/016, not TASK-014 -- the corrected split keeps
faith with that instead of quietly dropping it.

**Design decisions settled in the same pass, each recorded at the task
that first needs it rather than floated separately:**

- Field storage is `torch.Tensor`, `torch.float64` by default -- the
  first module in the engine to actually hold a tensor, not just Python
...
- A collocated field's storage shape is tied to its mesh by
  *construction* (`CollocatedField.__init__` allocates it), not merely
  ...
  `InvalidMeshEntityError` was added to close for `Mesh` itself.
- Invalid cell ids on a field raise `Mesh`'s own `InvalidMeshEntityError`
  rather than a new exception type, since it's the identical failure
  condition at the identical valid range, not a new one.
- `VectorField.magnitude()` and `.component(index)` exist specifically
  because TASK-017 needs them -- computed once on the field, not left for
  the rendering code to reach into raw tensor storage itself.
- TASK-017 scopes down twice, in both cases stating the limit rather than
  quietly under-building: arrows are plain line segments (no arrowhead,
  reusing TASK-013's existing line-drawing primitive), and the legend is
  an unlabelled colour strip -- wgpu/pygfx's text-rendering support has
  never been checked live, and claiming rendered numeric labels without
  that check would violate the same Integrity standard `docs/
  practices.md` already states elsewhere on this project.
- TASK-017's own config surface (`FieldDisplayConfig`, a handful of named
  demo patterns like `"radial_gradient"`) is explicitly narrower than
  `Field`'s general-callable Python API, and says so -- YAML can't carry
  a callable, and a safe expression parser is scope Stage 2 doesn't need
  just to satisfy the golden-demo public-API rule.

**Blast radius:** none outside `docs/planning/roadmap.md` -- no code
exists yet for these tasks, so no `CLAUDE.md`, inventory, or manifest
entry references anything this pass introduced.

- *Verified by:* no code changed; `make check-docs`, `make lint`,
  `make check-docs-index` and `make check-manifest` all run clean over
  the edited file.

### TASK-014 (Field Interface) implemented, strict TDD

Maintainer's request: start TASK-014 for real, and for this and every
task after it, write tests that check the intent behind the
functionality, not only mechanical input validation. Strict TDD, per
`docs/practices.md`: `tests/unit/test_field.py` written and confirmed
red (`ModuleNotFoundError: No module named 'pyflow.engine.field'`)
before `src/pyflow/engine/field.py` existed at all.

`Field`: a concrete `__init__(mesh, name)` (validated -- empty `name`
raises `ValueError`), read-only `mesh`/`name` properties, one abstract
method, `copy()`. Matches TASK-014's own Acceptance Criteria in
`roadmap.md` exactly.

Ten tests, aimed past the mechanical checks (constructor rejects an
empty name, a missing `copy()` implementation blocks instantiation) at
the actual invariants Stage 2's own Completion Criteria assert: that
mesh association is genuinely intrinsic per-instance rather than
class-level state (`test_distinct_fields_do_not_share_mesh_or_name` --
guards against the specific bug class where a shared mutable default or
accidental class attribute makes every field silently report whichever
mesh was set most recently), and that both properties are truly
immutable after construction, not merely undocumented as mutable
(`test_mesh_property_has_no_setter` / `test_name_property_has_no_setter`,
attempting the assignment and checking it actually raises, not just
reading the source).

**One real design correction, caught while drafting the Acceptance
Criteria last session, not during this implementation pass** -- recorded
here again because it changed what got built: `Field` carries no storage
at all, deliberately, so it makes no assumption about collocated vs.
staggered arrangement. See the previous changelog entry
("TASK-014..017 given real Acceptance Criteria...") for the full
reasoning; this entry is the correction actually landing in code.

`src/pyflow/engine/__init__.py` now re-exports `Field`, matching the
existing `CoordinateSystem`/`Mesh` convention.
`docs/repository-manifest.md`'s `src/` and `tests/` rows (file/test
counts, the file lists) and `roadmap.md`'s own live test-count paragraph
(202 → 212, same day) updated in the same change -- both are exactly the
kind of restated-fact-that-goes-stale `docs/CLAUDE.md` and this file's
own 2026-08-21 entries already warn about, so updating them alongside
the code rather than after was the point, not an afterthought.
`src/pyflow/engine/CLAUDE.md` gained `field.py`'s own entry, alongside
`coordinate_system.py`'s and `mesh.py`'s.

- *Verified by:* `make ci` clean end to end (212 tests, up from 202;
  100% coverage on the new module; `mypy --strict` clean;
  `check-docs`/`check-docs-index`/`check-graph`/`check-dependency-tree`/
  `check-inventory`/`check-manifest` all green) -- run once before this
  entry was written, not assumed from the pieces passing individually.

### TASK-015 (Scalar Field) implemented, strict TDD

Maintainer's follow-on request, same standing instruction as TASK-014:
write tests that check the intent behind the functionality, not only
mechanical input validation, for this and every task after it. Strict
TDD -- `tests/unit/test_field_contract.py` and `tests/unit/
test_scalar_field.py` written and confirmed red
(`ModuleNotFoundError: No module named 'pyflow.engine.collocated_field'`
/ `...scalar_field`) before either implementation module existed.

`CollocatedField(Field)`: storage allocation tied to the mesh by
construction (`(mesh.num_cells, *component_shape)`, never independently
settable), the generic initialiser (constant broadcast, or a callable
evaluated once per cell against `mesh.cell_centroid` -- the constant
case is simply the callable never being called, not a second code
path), and `_tensor_at`/`_set_tensor_at`, the id-checked tensor access
every leaf builds on, reusing `Mesh`'s own `InvalidMeshEntityError`
rather than a new exception type. `ScalarField(CollocatedField[float])`:
`component_shape = ()`, `value_at`/`set_value_at` converting through
those helpers to a plain `float`.

**One real typing correction, found by running `mypy --strict`, not
predicted while drafting the Acceptance Criteria.** The first
implementation gave `CollocatedField.value_at` a concrete body returning
`torch.Tensor`. `mypy --strict` rejected `ScalarField.value_at`'s
`float`-returning override outright (`float` and `Tensor` share no
subtype relationship, so this isn't the covariant-return case mypy
allows) -- three real errors, not a lint nitpick: `tests/unit/
test_field_contract.py` couldn't even see `.values`/`.set_value_at` on
what `field.copy()` returned, because `Field.copy` was typed `-> Field`
rather than `-> Self`. Two fixes, both structural rather than a
`# type: ignore`: `CollocatedField(Field, Generic[T])` now declares
`value_at`/`set_value_at` abstract over `T`, and `Field.copy` is typed
`-> Self` (`typing.Self`). Recorded in `src/pyflow/engine/CLAUDE.md` and
`roadmap.md` TASK-015 itself, not only here -- this changed what the
Artifacts Produced section there said `CollocatedField` provides
concretely, so that text was corrected in the same pass rather than left
to describe a design that was tried and abandoned.

**A smaller, procedural note on tool friction, worth recording because
it cost real turns:** the harness's own post-write formatting hook
strips an added-but-not-yet-used import (e.g. `from typing import Self`
added before any reference to `Self` existed in the file) between
separate `Edit` calls. Splitting "add the import" and "use the import"
across two tool calls lost the import each time. Fixed by making the
type-signature change and its import in one `Write` call instead of two
`Edit` calls -- worth remembering as the general pattern (single-file,
import-plus-usage changes go in one call) rather than re-discovering it
next time.

Contract suite design point worth stating since it wasn't obvious in
advance: the callable-initialiser contract test uses a **scalar**-valued
function (`x + 10*y`) broadcast into whatever `component_shape` is,
which keeps it implementation-independent across both `ScalarField` now
and `VectorField` later -- a function whose result genuinely differs
*per component* is `VectorField`'s own specific test (TASK-016), not the
shared contract's.

`src/pyflow/engine/__init__.py` now re-exports `CollocatedField` and
`ScalarField`. `docs/repository-manifest.md`'s `src/`/`tests/` rows and
`roadmap.md`'s live test-count paragraph (212 → 226) updated in the same
change, same reasoning as TASK-014's entry above.

- *Verified by:* `make ci` clean end to end (226 tests, up from 212; 14
...
### Stage 6's five design questions resolved, and TASK-042 split out

Put to the maintainer the day they were raised and answered directly.
Recorded in `docs/planning/roadmap.md` as questions-with-answers rather
than rewritten into decisions that read as though they were never in
doubt, following Stage 4's and Stage 5's precedent.

- **The stage gets a configuration task of its own, TASK-042 (Field
  Declaration Configuration), built first.** PyFlow could not declare a
  second transported field at all, so no criterion about adding one
  could be discharged. Unlike TASK-040 and TASK-041, which were both
  added mid-stage after the work had already landed inside another task
  by default, this one was visible in advance -- in the shape of a
  criterion no task could own. It is what makes "the last two tasks add
  zero lines under `src/pyflow/`" a measurement rather than an
  aspiration.
- **"No new machinery" means no new interface**, and no change to an
  existing one's signature. Implementing `SourceTerm` is the interface
  arriving at the consumer Stage 5 predicted for it in that module's own
  docstring. The engine diff this stage is allowed is bounded in advance
  to three things -- the derivative's source contribution, a seventh
  registry, the `AssembledNumerics` field carrying it -- and measured
  against the real diff at the exit audit. The honest cost is stated
  rather than left for the audit: TASK-035 is not a zero-engine-change
  task, and the stage's claim is now about its other four.
- **Density is Boussinesq**, and what that excludes is written down so
  its absence cannot later be read as a gap: no generalised continuity,
  no second pressure equation, and "conserves mass" means the density
  field's own domain integral. The stronger reading the 2026-08-22
  intent admits is a rewrite of Stage 5's solver inside a stage whose
  goal is that nothing changes.
- **A concrete buoyancy implementation lives under
  `src/pyflow/physics/`**, with the `SourceTerm` interface staying in
  `engine/numerics/`. The repository had already decided this --
  `physics/CLAUDE.md`'s own list names "buoyancy coupling" -- but the
  *split* had not been recorded, and it makes this the first
  implementation of a numerics interface to sit outside
  `engine/numerics/`. Recorded in that file where a reader meets it.
- **Rayleigh-Bénard's critical Rayleigh number is not this stage's
  bar.** The qualitative onset check is: rolls form heated from below
  and do not heated from above. Same reasoning Stage 5 applied to
  ADR-007's illustrative 2% for Ghia. The number is reassigned, not
  discarded -- `implementation-plan.md` and the backlog both say so now,
  rather than reading as though Stage 6 owed it.

**A sixth question came out of writing the task entries, and it
invalidated a criterion.** `SourceTerm.source(self, field: Field)`
receives only the field being advanced -- but buoyancy contributes to
momentum's `velocity.1` and is computed from the *temperature* field,
which that signature never passes it. So the interface Stage 3 designed
for exactly this consumer could not express its first consumer, and
Criterion 2 as drafted ("no existing interface's signature changes")
forbade the only fix. Found by tracing the call through
`simulation.step`'s own `derivative`, not by reading the interface.
**Resolved: widen to `source(self, field, state: Mapping[str, Field])`**
-- `SourceTerm` has no implementations so nothing breaks, the dependency
becomes explicit in a type rather than hidden in a closure, and
`derivative` already receives the intermediate state, so the term sees
each RK4 stage's own temperature for free. Rejected: binding the term to
a state inside `step` (a concept with no precedent here), and
constructing it with the driving field once per step (a stale reference
inside RK4's four stages, so a first-order splitting whose cost would
have to be defended). **Criterion 2's headline is amended rather than
quietly satisfied** -- it now reads "exactly one existing interface's
signature changes, named here in advance", which is the stronger shape:
the count is the constraint, and the exit audit measures the diff
against it. The criterion's own escape clause ("if it must change, that
is a design session, not an edit") is what was used, on the day the need
was found rather than during TASK-035's implementation. Recorded in
`src/pyflow/engine/numerics/source.py`'s own docstring where whoever
implements it will meet it.

Five task entries written to the six-section shape, each naming its own
`.feature` file as its acceptance criteria per ADR-007. TASK-035's entry
carries the **Boussinesq sign derivation worked in both directions**
rather than left to the implementer: one expression, `c (phi - phi_0) g`,
with `c = -beta` for temperature and `c = +1/rho_0` for density, checked
to give a rising warm parcel and a sinking dense one under a downward
`g`. That is what makes Criterion 4's "one coupling, not one per field"
a claim about the code rather than a hope, and the sign is exactly the
defect the 2026-08-18 review found in this repository's own prose.

**Two findings, both from comparing documents rather than reading one.**

**The Stage 6 golden-demo lists disagreed, and the first write-up of
that disagreement was itself wrong.** The criteria as committed said all
three demos were missing from the Golden Demos table and
`planning/data/demos.yaml`. Opening `demos.yaml` to add them showed Heat
Transport already carried there, as a second `validates` edge on
`demo-heat-diffusion` and a parenthetical on that demo's table row --
the reuse-at-a-later-Level pattern Taylor-Green follows. The real gap
was two: Smoke Transport (named only in Level 3's Golden Demo prose) and
Thermal Buoyancy (named only in the roadmap, and cited from
`implementation-plan.md` as "already named" there, so absent from Level
3's own list as well). Both now have table rows and graph entities;
Level 3's list names all three. The correction is recorded in the
criterion itself, because a criterion about comparing lists that was
written from two of the three lists is the same defect one level up.

**`check_references.py` had never checked `.feature` paths at all.**
`.feature` was not in its extension tuple, so no feature path in any
document was ever checked -- while Stage 5 listed four of them in that
script's `PLANNED` table on 2026-08-28 under a comment calling them
checked promises. Those entries could not have fired. Found because
Stage 6's criteria named five feature files that do not exist and the
gate said nothing. Fixed, with
`tests/unit/test_check_references.py::test_a_feature_file_is_a_checked_path`
as the regression, **verified by reverting the one line and watching it
fail** rather than by reading the code. Generalised into
`docs/practices.md`'s new "A rule that matches nothing reports nothing":
this is the third instance of that shape in two days, after the status
generator's hard-wrapped scenario pattern and `check_scenarios.py`'s own
reason for existing. Its corollary is the part worth keeping -- an
exemption or promise table is only as real as the scan that consults it,
and should be assumed inert until something has been seen to fail
because of an entry in it.

**A third finding, smaller and aimed squarely at TASK-042's
implementer: there are two configuration fields called
`scalar_pattern`.** `SimulationConfig`'s seeds a live transported field
and is the one that migrates; `FieldDisplayConfig`'s seeds one static
rendered frame, takes a different set of values, and must keep working
untouched -- `examples/golden-demos/field_display.yaml` sets it. Found
by checking which demo configs actually set the field being migrated
rather than assuming. Both obvious implementations of the loud break are
wrong on this point: a rejection keyed on the bare key name breaks that
demo, and a message reading "`scalar_pattern` has moved" misleads
whoever is reading it about the other one. Recorded in TASK-042's design
decisions, with a scenario of its own in that task's acceptance criteria
so the criterion cannot be satisfied by the broken version.

**A fourth, in this pass's own prose: "the seventh stage in a row" was
wrong.** Stage 6's criteria paragraph opened by copying the shape of the
claim Stage 5's own commit message made ("the sixth stage in a row"),
without opening Stage 1's section to check it -- which says "Written
2026-08-21, after the fact", that retrospective audit being the reason
the rule exists at all. Stage 0's criteria predate the rule too. The
real count is five (Stages 2, 3, 4, 5, 6), and the corrected paragraph
says why the obvious number differs, so the next stage does not inherit
the same off-by-two. Counting stages-with-criteria and counting
stages-that-followed-the-rule are different counts.

The ten Stage 6 artifacts those criteria and task entries promise are
now in `PLANNED` and genuinely checked: five feature files, one source
module (`src/pyflow/physics/buoyancy.py` -- named because design
question four settles it in advance, where Stage 5 deliberately named no
source module because its own question one was still open), three demo
configs, and `adr/ADR-010-source-term-state.md`. That last one is the
Blast Radius half of question six: this repository has recorded a Stage
3 interface signature change as an ADR twice (ADR-008 for
`TimeIntegrator.advance`, ADR-009 for `PressureCoupling.correct`), both
