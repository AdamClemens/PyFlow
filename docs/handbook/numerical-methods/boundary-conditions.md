...
particular, represents no flux at all -- an insulated wall, or a symmetry
plane). Numerically, the boundary's diffusive flux is computed directly
...
boundary condition (for example, an over-constrained or physically
inconsistent combination) can make the discrete linear system pressure-
velocity coupling produces ill-posed or poorly conditioned
(`linear-solvers.md`), independent of anything about the interior
discretisation.
