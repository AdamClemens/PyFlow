...
- [x] **`planning/model/*.yaml` and `planning/data/*.yaml`** -- the
      machine-readable knowledge graph. **Resolved 2026-08-21** (see the
      closing note at the end of this item). Originally: eleven empty
      files, deferred
      ...
      **Closed 2026-08-21: option 2, narrow the ADR.**
      `adr/ADR-006-knowledge-graph-scope.md` records the decision and its
      reasoning; `planning/` holds a real graph; `make check-graph` gates
      in `make ci`. The narrowing turns on one sentence of ADR-001 --
      "roadmaps and release plans will be generated" -- which cannot
      survive `roadmap.md` being 1,457 lines of reasoning. What the graph
      ...
      Three things it paid for immediately: the `dependency-tree.md`
      divergence below is resolved by construction; Capability Level 7's
      missing Stage is now a declared `unresolved:` that CI understands
      rather than something three documents mention and somebody has to
      remember; and building `demos.yaml` surfaced that
...
- [x] **`docs/planning/dependency-tree.md`: hand-maintained or derived?**
      **Derived, from 2026-08-21.** It is generated by
      `tools/generators/generate_dependency_tree.py` from
      `planning/data/components.yaml`, and `make check-dependency-tree`
      (in `make ci`) fails if the committed copy is stale. The note below
      about needing to "pick one shape" was exactly right, and the shape
      picked is `docs/architecture/engine.md`'s nine conceptual layers --
      `adr/ADR-006-knowledge-graph-scope.md` made that call, on the
      grounds that `engine.md` is the architecture document and the tree
      was a hand-drawn view of it. One correction to the framing:
      the engine is a DAG, not a tree (Flux alone depends on five other
      layers), so the generated document shows dependency *order* plus
      each component's direct dependencies rather than an ASCII tree --
      a tree can only draw a node once, which is part of why the
      hand-drawn one kept diverging. Original entry follows.
