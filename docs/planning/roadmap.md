...
| TASK-002 Build System | **Done** 2026-08-15 -- all targets run for real (eleven as of 2026-08-17's `check-docs`/`check-docs-index` additions); `lint` now runs the full pre-commit suite, `clean` states what it can't remove and why, new `ci` target added; `docs` is no longer a placeholder -- it regenerates `docs/index.md`, and `check-docs-index` fails CI if that file is stale |
