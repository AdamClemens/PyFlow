...
- *Forecloses:* the multi-renderer ambition for this data path, and
  ...
  replaceable with a NumPy implementation the way CuPy is).
- *Enables:* the only ✅-throughout path to GPU execution with rendering
  ...
  is no interop layer to build or debug.
...
1. Current headless/offscreen rendering support for VisPy, wgpu/pygfx,
   and Taichi GGUI specifically -- this could be disqualifying for D5,
   not just inconvenient, and should be checked before ruling any class
   in or out.
